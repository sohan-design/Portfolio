...
export default function ChatComposer({ fill = false }: { fill?: boolean }) {
  ...
  return (
    <div
      className={`flex w-full flex-col self-start overflow-hidden rounded-[14px] bg-surface shadow-card ${
        fill ? "h-full max-w-none" : "h-[288px] max-w-95"
      }`}
    >
      {/* header — tabs + actions */}
