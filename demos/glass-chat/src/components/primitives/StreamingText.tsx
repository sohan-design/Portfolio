...
const ACTION_ICONS: { key: string; label: string; icon: ReactNode }[] = [
  {
    key: "copy",
    label: "Copy",
    icon: (
...
export default function StreamingText({
  ...
  onAction?: (key: "copy" | "retry" | "up" | "down", plainText: string) => void;
} = {}) {
      ...
      >
        {ACTION_ICONS.map((action) => (
          <button
            ...
            onClick={() =>
              onAction?.(action.key as "copy" | "retry" | "up" | "down", plainText)
            }
